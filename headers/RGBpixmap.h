...
// fixed 11/7/98 to prevent OpenGL from trying to align pixels to four byte
// boundaries in memory which it does by default - see p.299.
...
public:
  ...
  void print() {
    cout << "IntRect: " << left << " " << top << " " << right << " " << bott
         << endl;
  ...
  void fix() { // reestablish order of left, right, etc.
    if (left > right) {
      int tmp = left;
      left = right;
      right = tmp;
...
public:
  ...
  //<<<<<<<<<<<<<<<<<< copy >>>>>>>>>>>>>>>>>>>
  void copy(IntPoint from, IntPoint to, int x, int y, int width,
            int height) { // copy a region of the display back onto the display
    if (nRows == 0 || nCols == 0)
      return;
  ...
  //<<<<<<<<<<<<<<<<< read >>>>>>>>>>>>>>>>
  int read(int x, int y, int wid,
           int ht) { // read a rectangle of pixels into this pixmap
    nRows = ht;
    nCols = wid;
  ...
  //<<<<<<<<<<<<<<<<< read from IntRect >>>>>>>>>>>>>>>>
  int read(IntRect r) { // read a rectangle of pixels into this pixmap
    nRows = r.top - r.bott;
